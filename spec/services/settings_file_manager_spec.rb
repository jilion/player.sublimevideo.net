...
describe SettingsFileManager do
  let(:site) do
    double('Site', token: 'abcd1234', hostname: 'google.com',
                   ...
                   default_kit: double('Kit', identifier: '1'))
  end
  let(:service) { described_class.new(site, 'stable') }

  describe '#settings_file' do
    before { site.stub(:kits) { [] } }

    context 'stable stage' do
      it 'has the right path' do
        service.settings_file.path.to_s.should eq "s3/abcd1234.js"
      end
    end

    context '"beta" stage' do
      let(:service) { described_class.new(site, 'beta') }

      it 'has the right path' do
        service.settings_file.path.to_s.should eq "s3/abcd1234-beta.js"
      end
    end

    context '"alpha" stage' do
      let(:service) { described_class.new(site, 'alpha') }

      it 'has the right path' do
        service.settings_file.path.to_s.should eq "s3/abcd1234-alpha.js"
      end
    end

    it 'concatenate all the needed package' do
      service.settings_file.file.read.gsub(/\s+\Z/, '').should eq <<-EOF.gsub(/^\s+/, '').gsub(/\s+\Z/, '')
        /*! SublimeVideo settings  | (c) 2013 Jilion SA | http://sublimevideo.net
        */(function(){ sublime_.define("settings",[],function(){var e,t,i;return t={},e={},i={license:{"hosts":["google.com","google.fr","google.ch"],"wildcard":true,"path":"foo","stage":"stable","stagingHosts":["staging.google.com"],"devHosts":["staging.google.com"]},app:{},kits:{},defaultKit:"1"},t.exports=i,t.exports||e});;sublime_.component('settings');})();
      EOF
    end

    it 'sets the right headers' do
      service.settings_file.headers.should eq({
